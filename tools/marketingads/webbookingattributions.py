...
__all__ = ["register_marketingads_web_booking_attributions_tools"]


def register_marketingads_web_booking_attributions_tools(mcp: Any) -> None:
    @mcp.tool()
    async def marketingads_web_booking_attributions_create(
        tenant: int,
        ...
        Mirrors WebBookingAttributions_Create.
        """
        ...
        url = f"{base_url}/marketingads/v2/tenant/{tenant}/web-booking-attributions"
        ...
        body = {"webSessionData": web_session, "bookingId": int(booking_id)}
        data = await make_st_post(url, json_body=body)
        if not data:
            return "Unable to create web booking attribution."
