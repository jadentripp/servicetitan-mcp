...
__all__ = ["register_marketingads_web_lead_form_attributions_tools"]


def register_marketingads_web_lead_form_attributions_tools(mcp: Any) -> None:
    @mcp.tool()
    async def marketingads_web_lead_form_attributions_create(
        tenant: int,
        ...
        Mirrors WebLeadFormAttributions_Create.
        """
        ...
        url = f"{base_url}/marketingads/v2/tenant/{tenant}/web-lead-form-attributions"
        ...
        body = {"webSessionData": web_session, "leadId": int(lead_id)}
        data = await make_st_post(url, json_body=body)
        if not data:
            return "Unable to create web lead form attribution."
