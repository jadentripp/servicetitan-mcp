...
__all__ = ["register_inventory_warehouses_tools"]


def register_inventory_warehouses_tools(mcp: Any) -> None:
    @mcp.tool()
    async def inventory_warehouses_get_list(
        tenant: int,
        ...
        """Get a paginated list of warehouses with filters. Mirrors Warehouses_GetList.
        ...
        url = f"{base_url}/inventory/v2/tenant/{tenant}/warehouses"
        ...
        if not data:
            return "Unable to fetch warehouses."
    ...
    @mcp.tool()
    async def inventory_warehouses_update(
        tenant: int,
        id: int,
        ...
        """Update a warehouse's external data. Mirrors Warehouses_Update."""

        base_url = get_base_url(environment)
        url = f"{base_url}/inventory/v2/tenant/{tenant}/warehouses/{id}"
        ...
        if not data:
            return "Unable to update warehouse."
